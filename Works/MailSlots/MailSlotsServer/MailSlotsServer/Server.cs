                ...
                if (MessageCount > 0)
                    for (int i = 0; i < MessageCount; i++)
                    {
                        ...
                        foreach (string user in connectedUsers.ToList())
                        {
                            ...
                            int mailSlotSender = DIS.Import.CreateFile(name, DIS.Types.EFileAccess.GenericWrite, DIS.Types.EFileShare.Read, 0, DIS.Types.ECreationDisposition.OpenExisting, 0, 0);
                            if (mailSlotSender == -1)
                            {
                            ...
                            DIS.Import.WriteFile(mailSlotSender, buff2, Convert.ToUInt32(buff2.Length), ref BytesWritten, 0);         // выполняем запись последовательности байт в канал
                            DIS.Import.CloseHandle(mailSlotSender);                 // закрываем дескриптор канала
                        }
